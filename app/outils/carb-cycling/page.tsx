...
// METADATA DÉPLACÉ DANS layout.tsx (voir fichier séparé)
...
export default function CarbCyclingUltraProPage() {
  // --- STATES ---
  ...
  const calculateCycle = () => {
    ...
    let fLow = Math.max(Math.round(w * 1.0), minFat);
    const remainingLow = kcalLow - (pLow * 4) - (fLow * 9);
    let cLow = Math.max(Math.round(remainingLow / 4), 30);
    if (remainingLow < 0) { fLow = Math.round((kcalLow - (pLow * 4) - 120) / 9); cLow = 30; }

    const pHigh = Math.round(lbm * pHighRatio);
    const fHigh = Math.max(Math.round(w * 0.5), minFat);
  ...
  const jsonLdSoftware = {
    ...
    "name": "Calculateur Carb Cycling Pro STRYV LAB",
    ...
    "description": "Calculateur de carb cycling professionnel basé sur la masse maigre (LBM). Protocole validé ISSN et Dr. Layne Norton pour optimiser la sèche sans perdre de muscle.",
    "featureList": [
      "Calcul BMR Mifflin-St Jeor",
      "PAL personnalisé selon occupation et entraînement",
      "Protéines basées sur masse maigre (LBM)",
      "Méthode US Navy pour body fat",
      "Lipides minimums hormonaux",
      "Protocoles 2/1, 3/1, 4/1, 5/2",
      "Ajustement sensibilité insulinique",
      "Export PDF/Copie résultats"
    ],
    "aggregateRating": {
  ...
  return (
    <>
      ...
      <main className="flex flex-col lg:flex-row min-h-screen font-outfit text-white">
        ...
        {/* ================= GAUCHE (Intacte + SEO Description) ================= */}
            ...
            {result && (
                <div className="animate-in fade-in slide-in-from-bottom-12 duration-700 space-y-6 mt-16 max-w-3xl mx-auto">
                    ...
                    <ResultActions 
                        data={{
                            tdee: result.tdee,
                            goal: getGoalLabel(goal) || 'Objectif',
                            protocol: protocol,
                            macros: {
                                high: { cal: result.high.kcal, p: result.high.p, f: result.high.f, c: result.high.c },
                                low: { cal: result.low.kcal, p: result.low.p, f: result.low.f, c: result.low.c }
                            }
                        }} 
                    />
                </div>
